  ...
  protected void doServe() throws IOException {
    try {
      ...
      try {
        ...
        LOGGER.error("Database error while retrieving user with id %d: %s", userId, e.getMessage());
      ...
      new Message("Invalid user ID: " + req.getAttribute("user_id"),
          ErrorCodes.INVALID_INPUT_PARAMETER, e.getMessage()).toJSON(res.getOutputStream());
    }
  }
}
