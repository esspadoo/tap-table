 ...
 * 
 * <p>
 * The {@code id}, passed as a path parameter, is used to identify the ingredient to be retrieved.
 * If the id provided is numerical only (alphanumerical), the response status is set to {@code 400}
 * (Bad Request).
 * </p>
 * 
 * <p>
 * If the ingredient is successfully retrieved, the response status is set to {@code 200} (OK) and
 * the ingredient is returned in the response body as a JSON object.
 * </p>
 * 
 * <p>
 * If the ingredient with the specified id does not exist, the response status is set to {@code 404}
 * (Not Found).
 * </p>
 * 
 * <p>
 * If any server side error occurs (e.g. database) during the retrieval process, the response status
 * is set to {@code 500} (Internal Server Error).
 * </p>
 * 
 * @author SWAD Team
 */
  ...
  protected void doServe() throws IOException {
    try {
      ...
      if (ingredient == null) {
        ...
        Message m = new Message(String.format("Ingredient %d not found in database", ingredientId),
      ...
      Message m = new Message("Database error while retrieving ingredient",
      ...
      Message m = new Message("Ingredient id must be a valid integer",
