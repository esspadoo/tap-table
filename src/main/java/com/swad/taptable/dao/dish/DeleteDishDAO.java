...
public class DeleteDishDAO extends AbstractDAO<Boolean> {
    ...
    protected void doAccess() throws SQLException {
        ...
        try (PreparedStatement pstmt = con.prepareStatement(STATEMENT)) {
            ...
            /**
             * PreparedStatement::executeUpdate() returns the number of rows affected by the query.
             * If it returns 1, this means that one row was deleted, which it was intended.
             * Otherwise, if it returns 0, it means that no rows were deleted, which could indicate
             * that the dish with the provided ID doesn't exist
             */
