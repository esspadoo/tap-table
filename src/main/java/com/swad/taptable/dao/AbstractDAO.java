...
import com.swad.taptable.util.ConnectionPoolSingleton;
 ...
 *
 * <p>{@code AbstractDAO} is designed to achieve the following goals:
 *
 * <ol>
 *   <li>a consistent way of accessing the database and managing exceptions is guaranteed for all of
 *       the classes that need to directly interact with the DBMS.
 *   <li>independence from any particular DBMS is achieved, meaning that the DBMS-dependent code is
 *       as isolated as possible and that upper layers can transparently change the underlying DBMS,
 *       without the need of modifying their code.
 * </ol>
 *
 * <p>With respect to the first goal, {@code AbstractDAO} provides a basic infrastructure which
 * manages the connection, the commitment of transactions (or the roll-back), if needed, and any
 * exception that may be thrown while accessing the database.
 *
 * <p>With respect to the second goal,{@code AbstractDAO} is the root of a hierarchy of helper
 * classes, which isolate as much as possible the DBMS-dependent code.
 *
 * <p>Concrete subclasses have to implement the abstract method {@link #doAccess()} in order to
 * define the needed logic for accessing the database. This method is, in turn, called by {@link
 * AbstractDAO#access()}, which provides a consistent way of accessing the database and managing
 * exceptions.
 *
 * <p>Concrete sub-classes may also override {@link AbstractDAO#access()} in order to provide a
 * different management of connection and transaction if they have peculiar needs. Nevertheless, the
 * objective of this class is to ease sub-classes by providing a pre-packed logic for connection and
 * transaction management, that is what {@link AbstractDAO#access()} does.
 *
...
public abstract class AbstractDAO<T> implements DataAccessObject<T> {
   ...
   *
   * <p>The actual logic for accessing the database is defined in {@link #doAccess()}, which is
   * called by this method.
   *
   * <p>If any kind of exception is thrown while accessing the database, it is caught and handled in
   ...
   * method as well.
   *
   * <p>If the access method throws an SQLException, it is re-thrown to be handled by the upper
   * layers. If it throws any other kind of exception, it is wrapped in an SQLException and then
   * re-thrown.
   */
  public final DataAccessObject<T> access() throws SQLException {
    ...
    /**
     * The logic is the following: the actual access to the database is performed by {@link
     * #doAccess()}, which is called in the try block. If any exception is thrown while accessing
     * the database, it is caught in the catch block, where the transaction is rolled back, if
     * needed, and the connection is closed. If the access to the database is successful, the
     * connection is closed in the try block.`
     */
    try {
      doAccess();
   ...
   *
   * <p>Subclasses have to implement this method in order to define the actual strategy for
